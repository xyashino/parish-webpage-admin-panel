...
const findActiveTable = (intentions: DayIntentionsResponse[], menu: MenuItem[]) => {
    return (
...
export const IntentionsPreviewPage = () => {
    ...
    const [table, setTable] = useState<DayIntentionsResponse>(
        findActiveTable(intentions, menu)
    );

    const updateTable = (type: string) => {
        setMenu((prevState) => {
            prevState.forEach(
                (el) =>
                    (el.active =
        ...
        setTable(() => findActiveTable(intentions, menu));
    };
    const title = (
        <p>
    ...
    const panel = (
        <div className="card w-full">
            <Menu items={menu} onClick={updateTable} />
        </div>
    );
    ...
    return (
        <section className="flex w-5/6 flex-col items-center bg-base-200">
            ...
            />
            <GridLayoutPage titleContent={title} panelContent={panel}>
